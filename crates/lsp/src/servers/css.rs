...
/// Candidate for `vscode-css-language-server`, shipped in the same npm package
/// as the HTML and JSON servers (`vscode-langservers-extracted`). We manage a
...
pub struct CssLanguageServerCandidate {
    client: Arc<http_client::Client>,
}

impl CssLanguageServerCandidate {
    /// Path to the CSS server launcher JS within the npm package, relative to
    ...
    /// Finds the configuration for running vscode-css-language-server from our
    ...
    pub async fn find_installed_binary_config(
        ...
        if !server_js.is_file() {
            log::info!(
                "vscode-css-language-server launcher not found at {}",
                server_js.display()
            );
            return None;
        }
...
#[async_trait]
#[cfg(feature = "local_fs")]
impl LanguageServerCandidate for CssLanguageServerCandidate {
    ...
    async fn install(
        ...
        log::info!(
            "Installing vscode-langservers-extracted (CSS) version {}",
            metadata.version
        );
        ...
        log::info!("vscode-css-language-server installed successfully");
        Ok(())
    }
