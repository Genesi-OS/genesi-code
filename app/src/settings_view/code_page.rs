...
const CODEBASE_INDEX_DESCRIPTION: &str = "Genesi Code can automatically index repositories as you navigate them, helping the agent understand project context and provide targeted solutions. Your source code remains local.";
const WARP_INDEXING_IGNORE_DESCRIPTION: &str = "Files excluded from indexing remain accessible to local agent tools, but are not included in codebase embeddings.";
const AUTO_INDEX_FEATURE_NAME: &str = "Index new folders by default";
const AUTO_INDEX_DESCRIPTION: &str = "Automatically index repositories as you navigate them so Genesi AI can understand project context.";
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CodeSettingsPageAction::ToggleAutosave => {
                CodeSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.autosave_enabled.toggle_and_save_value(ctx));
                });
                ctx.notify();
            }
...
impl SettingsWidget for AutosaveToggleWidget {
    ...
    fn search_terms(&self) -> &str {
        "autosave auto save ide editor save files automatically"
    }
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            ...
            appearance
                .ui_builder()
                .switch(self.switch_state.clone())
                .check(*code_settings.autosave_enabled)
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(CodeSettingsPageAction::ToggleAutosave);
                })
                .finish(),
            Some("Automatically saves IDE file edits shortly after you stop typing.".into()),
        )
    }
}
