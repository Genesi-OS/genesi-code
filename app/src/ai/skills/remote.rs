...
impl SkillManager {
    ...
    fn bootstrap_remote_bundled_skill(&mut self, host_id: HostId, ctx: &mut ModelContext<Self>) {
        ...
        let Some(resources_dir) = RemoteServerManager::as_ref(ctx)
        ...
        let Ok(resources_dir) = StandardizedPath::try_new(&resources_dir) else {
            log::warn!(
                "Remote host {host_id} advertised an invalid bundled resources directory: {resources_dir}"
            );
            return;
        };
        let resources_dir =
