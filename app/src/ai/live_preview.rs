...
fn convert_node(handle: &Handle) -> Option<DomNode> {
    match &handle.data {
...
pub fn compile_html(project_root: &Path, path: &Path, html: &str) -> PreviewDocument {
    ...
    let root = compiler.compile_element(&body, &root_style, &ancestry, &HashMap::new());
...
fn collect_landmarks(element: &DomElement, out: &mut Vec<(String, String)>, depth: usize) {
    ...
    for child in &element.children {
        ...
        if is_landmark {
            ...
            out.push((label, serialize_element(child)));
            if out.len() >= 24 {
                return;
            }
        }
...
fn serialize_element(element: &DomElement) -> String {
    ...
    for child in &element.children {
        match child {
            DomNode::Text(text) => out.push_str(text),
            DomNode::Element(child) => out.push_str(&serialize_element(child)),
        }
    }
